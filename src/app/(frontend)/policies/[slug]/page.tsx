...
export default async function Policy({ params: paramsPromise }: Args) {
  const { slug = '' } = await paramsPromise
  const url = '/policies/' + slug
  const policy = await queryPolicyBySlug({ slug })
...
const queryPolicyBySlug = cache(async ({ slug }: { slug: string }) => {
